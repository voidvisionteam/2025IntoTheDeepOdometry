...
@Autonomous(name = "Auto_Framework2",group = "Autonomous")
public  class Auto_Framework2 extends Auto_Util {
    ...
    public class Clock{
        public class WAIT implements Action {
            ...
            public boolean run(@NonNull TelemetryPacket packet) {
                if (!started) {
                    timer.reset();  // Reset the timer when the action starts
                    started = true;
                }
                return timer.seconds() < waitTime;  // Return true while waiting, false when done
            }


            public void stop(@NonNull TelemetryPacket packet) {
                // Optional: Cleanup if needed when the action is interrupted
            }
        }

        public Action NonBlockingWait(double seconds) {
